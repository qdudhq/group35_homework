...
void ctr_crypt(const uint8_t *in, uint8_t *out, size_t nbytes,
    ...
    if (nonce_len > AES_BLOCK_SIZE) nonce_len = AES_BLOCK_SIZE;
    ctr_len = AES_BLOCK_SIZE - nonce_len;
    ...
    while (nbytes > 0) {
        ...
        size_t chunk = (nbytes < AES_BLOCK_SIZE) ? nbytes : AES_BLOCK_SIZE;
        for (size_t i = 0; i < chunk; i++)
            out[i] = in[i] ^ keystream[i];

        /* 递增计数器 (big-endian, 最后的 ctr_len 字节) */
        for (int i = (int)(AES_BLOCK_SIZE - 1); i >= (int)nonce_len; i--) {
            if (++ctr[i] != 0) break;
        }
