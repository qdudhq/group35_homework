/* sm4_modes.c - adapted from AES modes */
/*
 * modes.c - 工作模式实现: CTR, GCM, XTS
 *
 * 所有模式接受通用函数指针, 可对接任意的 AES 实现
 * (参考/T-table/Shuffle/SM4-NI)
 *
 * GCM 支持两种 GHASH 实现:
...
void sm4_ctr_crypt(const uint8_t *in, uint8_t *out, size_t nbytes,
    ...
    if (nonce_len > SM4_BLOCK_SIZE) nonce_len = SM4_BLOCK_SIZE;
    ctr_len = SM4_BLOCK_SIZE - nonce_len;

    /* 初始化计数器块 */
    memset(ctr, 0, SM4_BLOCK_SIZE);
    memcpy(ctr, nonce, nonce_len);

    while (nbytes > 0) {
        encrypt(ctr, keystream, ctx);

        size_t chunk = (nbytes < SM4_BLOCK_SIZE) ? nbytes : SM4_BLOCK_SIZE;
        for (size_t i = 0; i < chunk; i++)
            out[i] = in[i] ^ keystream[i];

        /* 递增计数器 (big-endian, 最后的 ctr_len 字节) */
        for (int i = (int)(SM4_BLOCK_SIZE - 1); i >= (int)nonce_len; i--) {
            if (++ctr[i] != 0) break;
        }
