/*
 * sm4_ttable.c - SM4 T-table 优化实现
 *
 * 核心思想: 将 S-box 查表 + 线性变换 L 合并为 4 张查找表
 * T0, T1, T2, T3 各 256 个 uint32_t (共 4KB)
 *
 * 每轮: T(X1^X2^X3^rk) = T0[b0] ^ T1[b1] ^ T2[b2] ^ T3[b3]
 * 相比参考实现的性能提升: 约 5-10x
 */

#include "sm4.h"

/* ---- SM4 S-box ---- */
static const uint8_t SBOX[256] = {
    
};

/* 系统参数 FK 和固定参数 CK */
static const uint32_t FK[4] = {
    0xa3b1bac6U, 0x56aa3350U, 0x677d9197U, 0xb27022dcU
};
static const uint32_t CK[32] = {
    0x00070e15U,0x1c232a31U,0x383f464dU,0x545b6269U,
    0x70777e85U,0x8c939aa1U,0xa8afb6bdU,0xc4cbd2d9U,
    0xe0e7eef5U,0xfc030a11U,0x181f262dU,0x343b4249U,
    0x50575e65U,0x6c737a81U,0x888f969dU,0xa4abb2b9U,
    0xc0c7ced5U,0xdce3eaf1U,0xf8ff060dU,0x141b2229U,
    0x30373e45U,0x4c535a61U,0x686f767dU,0x848b9299U,
    0xa0a7aeb5U,0xbcc3cad1U,0xd8dfe6edU,0xf4fb0209U,
    0x10171e25U,0x2c333a41U,0x484f565dU,0x646b7279U
};

/* ---- 辅助: 循环左移 ---- */
static uint32_t rotl(uint32_t x, int n) {
    return (x << n) | (x >> (32 - n));
}

/* SM4 L: 线性变换 B ^ (B<<<2) ^ (B<<<10) ^ (B<<<18) ^ (B<<<24) */
static uint32_t L(uint32_t B) {
    return B ^ rotl(B, 2) ^ rotl(B, 10) ^ rotl(B, 18) ^ rotl(B, 24);
}

/* L': 密钥扩展用 */
static uint32_t Lprime(uint32_t B) {
    return B ^ rotl(B, 13) ^ rotl(B, 23);
}

/* ---- 预计算 T-tables ----
 * T0[x] = L(S(x) << 24)   ← 高字节贡献
 * T1[x] = L(S(x) << 16)   ← 次高字节
 * T2[x] = L(S(x) << 8)    ← 次低字节
 * T3[x] = L(S(x))         ← 低字节贡献
 *
 * 因为 L 是 GF(2) 上的线性函数:
 * L(b0||b1||b2||b3) = T0[b0] ^ T1[b1] ^ T2[b2] ^ T3[b3]
 */
static uint32_t T0[256], T1[256], T2[256], T3[256];
static int tables_initialized = 0;

static void init_tables(void) {
    int i;
    for (i = 0; i < 256; i++) {
        uint8_t s = SBOX[i];
        T0[i] = L((uint32_t)s << 24);
        T1[i] = L((uint32_t)s << 16);
        T2[i] = L((uint32_t)s << 8);
        T3[i] = L((uint32_t)s);
    }
    tables_initialized = 1;
}

/* ---- 密钥扩展 (标准方式, 与参考实现相同) ---- */
void sm4_ttable_init(sm4_ctx *ctx, const uint8_t *key) {
    uint32_t MK[4], K[36];
    int i;

    if (!tables_initialized) init_tables();

    for (i = 0; i < 4; i++) {
        MK[i] = ((uint32_t)key[4*i]   << 24) |
                ((uint32_t)key[4*i+1] << 16) |
                ((uint32_t)key[4*i+2] << 8)  |
                ((uint32_t)key[4*i+3]);
    }

    for (i = 0; i < 4; i++)
        K[i] = MK[i] ^ FK[i];

    for (i = 0; i < 32; i++) {
        K[i + 4] = K[i] ^ Lprime(
            ((uint32_t)SBOX[(K[i+1] >> 24) & 0xFF] << 24) |
            ((uint32_t)SBOX[(K[i+1] >> 16) & 0xFF] << 16) |
            ((uint32_t)SBOX[(K[i+1] >> 8)  & 0xFF] << 8)  |
            ((uint32_t)SBOX[ K[i+1]        & 0xFF])
            ^ K[i + 2] ^ K[i + 3] ^ CK[i]
        );
        ctx->rk[i] = K[i + 4];
    }
}

/* ---- T-table 加密 ----
 * 使用4张查找表, 每轮只需 4次查表+4次异或 (代替逐字节S-box+线性变换)
 */
void sm4_ttable_encrypt(const uint8_t *pt, uint8_t *ct, const sm4_ctx *ctx) {
    uint32_t X[36];
    int i;

    if (!tables_initialized) init_tables();

    /* 加载明文 */
    for (i = 0; i < 4; i++) {
        X[i] = ((uint32_t)pt[4*i]   << 24) |
               ((uint32_t)pt[4*i+1] << 16) |
               ((uint32_t)pt[4*i+2] << 8)  |
               ((uint32_t)pt[4*i+3]);
    }

    /* 32轮迭代 (T-table加速版) */
    for (i = 0; i < 32; i++) {
        uint32_t A = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ ctx->rk[i];
        uint32_t a0 = (A >> 24) & 0xFF;
        uint32_t a1 = (A >> 16) & 0xFF;
        uint32_t a2 = (A >> 8)  & 0xFF;
        uint32_t a3 =  A        & 0xFF;
        X[i + 4] = X[i] ^ T0[a0] ^ T1[a1] ^ T2[a2] ^ T3[a3];
    }

    /* 输出密文 (逆序) */
    for (i = 0; i < 4; i++) {
        uint32_t v = X[35 - i];
        ct[4*i]   = (uint8_t)(v >> 24);
        ct[4*i+1] = (uint8_t)(v >> 16);
        ct[4*i+2] = (uint8_t)(v >> 8);
        ct[4*i+3] = (uint8_t)(v);
    }
}

/* ---- T-table 解密 (轮密钥逆序) ---- */
void sm4_ttable_decrypt(const uint8_t *ct, uint8_t *pt, const sm4_ctx *ctx) {
    uint32_t X[36];
    int i;

    if (!tables_initialized) init_tables();

    /* 加载密文 */
    for (i = 0; i < 4; i++) {
        X[i] = ((uint32_t)ct[4*i]   << 24) |
               ((uint32_t)ct[4*i+1] << 16) |
               ((uint32_t)ct[4*i+2] << 8)  |
               ((uint32_t)ct[4*i+3]);
    }

    /* 32轮迭代 (轮密钥逆序: 31..0) */
    for (i = 0; i < 32; i++) {
        uint32_t A = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ ctx->rk[31 - i];
        uint32_t a0 = (A >> 24) & 0xFF;
        uint32_t a1 = (A >> 16) & 0xFF;
        uint32_t a2 = (A >> 8)  & 0xFF;
        uint32_t a3 =  A        & 0xFF;
        X[i + 4] = X[i] ^ T0[a0] ^ T1[a1] ^ T2[a2] ^ T3[a3];
    }

    /* 输出明文 (逆序) */
    for (i = 0; i < 4; i++) {
        uint32_t v = X[35 - i];
        pt[4*i]   = (uint8_t)(v >> 24);
        pt[4*i+1] = (uint8_t)(v >> 16);
        pt[4*i+2] = (uint8_t)(v >> 8);
        pt[4*i+3] = (uint8_t)(v);
    }
}
