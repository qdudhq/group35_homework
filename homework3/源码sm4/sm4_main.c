/*
 * sm4_main.c - SM4-128 Test & Benchmark
 *
 * 对比 4 种 SM4 实现 + 3 种工作模式:
 *   1. 参考实现
 *   2. T-table 优化
...
/* NOTE: SM4-128 only uses 128-bit keys.
...
/* GM/T 0002-2012 Appendix C.3: SM4-256 */
...
/* NIST GCM 测试向量 (SM4-128, nonce=12字节) */
...
static int test_ctr(const char *name, sm4_block_fn enc_fn, const void *ctx) {
    uint8_t ct[64], pt[64];
...
static void bench_ctr(const char *name, sm4_block_fn enc_fn, const void *ctx) {
...
/* ================================================================
 * SM4-128: 64-bit block cipher
 * Note: block arrays use [8], bench uses 8-byte blocks
 * ================================================================ */

/* Wrappers */
static void w_ref_enc(const uint8_t *in, uint8_t *out, const void *ctx)
{ sm4_ref_encrypt(in, out, (const sm4_ctx*)ctx); }
static void w_ref_dec(const uint8_t *in, uint8_t *out, const void *ctx)
{ sm4_ref_decrypt(in, out, (const sm4_ctx*)ctx); }
static void w_tt_enc(const uint8_t *in, uint8_t *out, const void *ctx)
{ sm4_ttable_encrypt(in, out, (const sm4_ctx*)ctx); }
static void w_tt_dec(const uint8_t *in, uint8_t *out, const void *ctx)
{ sm4_ttable_decrypt(in, out, (const sm4_ctx*)ctx); }
static void w_sh_enc(const uint8_t *in, uint8_t *out, const void *ctx)
{ sm4_shuffle_encrypt(in, out, (const sm4_ctx*)ctx); }
static void w_ni_enc(const uint8_t *in, uint8_t *out, const void *ctx)
{ sm4_ni_encrypt(in, out, (const sm4_ni_ctx*)ctx); }
static void w_ni_dec(const uint8_t *in, uint8_t *out, const void *ctx)
{ sm4_ni_decrypt(in, out, (const sm4_ni_ctx*)ctx); }

/* Self-consistency tests (8-byte blocks for SM4) */
static int test_rt(const char *n, sm4_block_fn enc, sm4_block_fn dec, const void *ctx) {
    uint8_t ct[16], pt2[8];
    enc(test_pt, ct, ctx);
    dec(ct, pt2, ctx);
...
static int test_cross(const char *n, sm4_block_fn e, sm4_block_fn d,
...
static int test_ctr_ok(const char *n, sm4_block_fn enc, const void *ctx) {
    uint8_t buf[64], tmp[64]; uint8_t nonce[4]={0}; int i;
    memset(buf, 0xAA, 64);
    sm4_ctr_crypt(buf, tmp, 64, nonce, 4, enc, ctx);
    sm4_ctr_crypt(tmp, buf, 64, nonce, 4, enc, ctx);
    for (i=0;i<64;i++) if(buf[i]!=0xAA) { printf("  FAIL: %s CTR\\n", n); return 0; }
    return 1;
}

static int test_gcm_ok(const char *n, sm4_block_fn enc, const void *ctx) {
    uint8_t pt[16], ct[16], pt2[16], tag[16]; uint8_t nonce[12]={0};
    memset(pt, 0x55, 32);
    sm4_gcm_encrypt(pt, ct, 32, NULL, 0, nonce, 12, tag, enc, ctx);
    int r=sm4_gcm_decrypt(ct, pt2, 32, NULL, 0, nonce, 12, tag, enc, ctx);
    if (r!=0||memcmp(pt,pt2,16)!=0) { printf("  FAIL: %s GCM\\n", n); return 0; }
    tag[0]^=1;
    if (sm4_gcm_decrypt(ct,pt2,16,NULL,0,nonce,12,tag,enc,ctx)==0) { printf("  FAIL: %s GCM tamper\\n", n); return 0; }
    return 1;
}

static int test_xts_ok(const char *n, sm4_block_fn enc, sm4_block_fn dec,
                       const void *k1, const void *k2) {
    uint8_t pt[48], ct[48], dpt[48]; int i;
    for (i=0;i<48;i++) pt[i]=(uint8_t)i;
    sm4_xts_encrypt(pt, ct, 48, 0, enc, enc, k1, k2);
    if (memcmp(pt, ct, 48)==0) { printf("  FAIL: %s XTS no change\\n", n); return 0; }
    sm4_xts_decrypt(ct, dpt, 48, 0, enc, dec, k1, k2);
...
static void bench_enc(const char *n, sm4_block_fn fn, const void *ctx) {
...
static void bench_ctr(const char *n, sm4_block_fn fn, const void *ctx) {
    uint8_t *buf=(uint8_t*)malloc(BENCH_MBYTES); uint8_t nonce[4]={0};
    memset(buf,0xAA,BENCH_MBYTES);
    sm4_ctr_crypt(buf,buf,BENCH_MBYTES,nonce,4,fn,ctx);
    double s=get_time_us();
    sm4_ctr_crypt(buf,buf,BENCH_MBYTES,nonce,4,fn,ctx);
    double e=get_time_us()-s;
...
static void bench_gcm(const char *n, sm4_block_fn fn, const void *ctx) {
    size_t sz=BENCH_MBYTES;
    uint8_t *buf=(uint8_t*)malloc(sz),*ctb=(uint8_t*)malloc(sz);
    uint8_t nonce[12]={0},tag[16];
    memset(buf,0xAA,sz);
    sm4_gcm_encrypt(buf,ctb,sz,NULL,0,nonce,12,tag,fn,ctx);
    double s=get_time_us();
    sm4_gcm_encrypt(buf,ctb,sz,NULL,0,nonce,12,tag,fn,ctx);
    double e=get_time_us()-s;
    printf("  %-20s  %8.1f MB/s\\n", n, (double)sz/e);
...
int main(void) {
...
#endif
    printf("============================================================\\n");
    printf("  SM4-128 Software Optimization - Experiment 3\\n");
    printf("============================================================\\n\\n");
    ...
    { sm4_ctx c; sm4_ref_init(&c,test_key);
      ok=test_rt("Ref",w_ref_enc,w_ref_dec,&c);
      printf("  Ref ........... %s\\n",ok?"PASS":"FAIL"); total+=ok; }
    { sm4_ctx c; sm4_ttable_init(&c,test_key);
      ok=test_rt("T-table",w_tt_enc,w_tt_dec,&c);
      printf("  T-table ....... %s\\n",ok?"PASS":"FAIL"); total+=ok; }
    { sm4_ctx c; sm4_shuffle_init(&c,test_key);
      ok=test_rt("Shuffle",w_sh_enc,w_ref_dec,&c);
      printf("  Shuffle ....... %s\\n",ok?"PASS":"FAIL"); total+=ok; }
    fflush(stdout);

    printf("\\n[Test 2] Cross-implementation\\n"); fflush(stdout);
    { sm4_ctx c1,c2; sm4_ttable_init(&c1,test_key); sm4_ref_init(&c2,test_key);
      ok=test_cross("TT->Ref",w_tt_enc,w_ref_dec,&c1,&c2);
      printf("  TT->Ref ....... %s\\n",ok?"PASS":"FAIL"); total+=ok; }
    { sm4_ctx c1,c2; sm4_ref_init(&c1,test_key); sm4_ttable_init(&c2,test_key);
    ...
    printf("\\n[Test 5] XTS mode\\n"); fflush(stdout);
    { sm4_ctx k1,k2; uint8_t xk[16]={0};
      sm4_ref_init(&k1,xk); sm4_ref_init(&k2,test_key);
      ok=test_xts_ok("Ref-XTS",w_ref_enc,w_ref_dec,&k1,&k2);
      printf("  Ref-XTS ....... %s\\n",ok?"PASS":"FAIL"); total+=ok; }
    { sm4_ctx k1,k2;
      sm4_ttable_init(&k1,test_key); sm4_ttable_init(&k2,test_key);
    ...
    /* Performance */
    printf("\\n============================================================\\n");
    printf("[Performance] SM4-128 single-block encrypt\\n");
    printf("============================================================\\n");
    fflush(stdout);
    { sm4_ctx c; sm4_ref_init(&c,test_key);
    ...
    { sm4_ni_ctx c; sm4_ni_init(&c,test_key);
      bench_enc("4. NI(SSE4.1)", w_ni_enc, &c); }
    { sm4_ctx c; sm4_avx2_init(&c,test_key);
      uint8_t p4[4][16],c4[4][16]; int w,i;
      memset(p4,0,sizeof(p4));
      for(w=0;w<WARMUP;w++) sm4_avx2_encrypt_x4(p4,c4,&c);
      double st=get_time_us();
      for(i=0;i<BENCH_BLOCKS/4;i++) sm4_avx2_encrypt_x4(p4,c4,&c);
      double e4=get_time_us()-st;
      printf("  %-20s  %8.1f MB/s\\n","5. AVX2 (4x)",(double)(BENCH_BLOCKS*16)/e4); }
    { sm4_ctx c; sm4_avx2_init(&c,test_key);
      uint8_t p8[8][16],c8[8][16]; int w,i;
      memset(p8,0,sizeof(p8));
      for(w=0;w<WARMUP;w++) sm4_avx2_encrypt_x8(p8,c8,&c);
      double st=get_time_us();
      for(i=0;i<BENCH_BLOCKS/8;i++) sm4_avx2_encrypt_x8(p8,c8,&c);
