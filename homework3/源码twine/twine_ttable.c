/* twine_ttable.c - TWINE-128 T-table optimization */
#include "twine.h"

static const uint8_t SB[16]={0xc,0x0,0xf,0xa,0x2,0xb,0x9,0x5,0x8,0x3,0xd,0x7,0x1,0xe,0x6,0x4};
static const uint8_t ISB[16]={0x1,0xc,0x4,0x9,0xf,0x7,0xe,0xb,0x8,0x6,0x3,0x5,0x0,0xa,0xd,0x2};
...
void twine_ttable_decrypt(const uint8_t *ct,uint8_t *pt,const twine_ctx *ctx){
    uint8_t B[16];
    for(int i=0;i<8;i++){B[2*i]=(ct[i]>>4)&0xF;B[2*i+1]=ct[i]&0xF;}
    for(int r=35;r>=0;r--){
        uint32_t rk=ctx->rk[r];
        for(int i=0;i<8;i++)B[i]^=(uint8_t)((rk>>(28-4*i))&0xF);
        uint8_t t=B[15];for(int i=15;i>0;i--)B[i]=B[i-1];B[0]=t;
        for(int i=0;i<8;i++)B[i+8]^=B[i];
        for(int i=0;i<8;i++)B[i]=ISB[B[i]];
    }
    for(int i=0;i<8;i++)pt[i]=(uint8_t)((B[2*i]<<4)|B[2*i+1]);
}
