/* twine_modes.c - adapted from AES modes */
/*
 * modes.c - 工作模式实现: CTR, GCM, XTS
 *
 * 所有模式接受通用函数指针, 可对接任意的 AES 实现
 * (参考/T-table/Shuffle/TWINE-NI)
 *
 * GCM 支持两种 GHASH 实现:
...
void twine_ctr_crypt(const uint8_t *in, uint8_t *out, size_t nbytes,
               ...
               twine_block_fn encrypt, const void *ctx) {
    uint8_t ctr[TWINE_BLOCK_SIZE];
    uint8_t keystream[TWINE_BLOCK_SIZE];
    size_t ctr_len;

    if (nonce_len > TWINE_BLOCK_SIZE) nonce_len = TWINE_BLOCK_SIZE;
    ctr_len = TWINE_BLOCK_SIZE - nonce_len;

    /* 初始化计数器块 */
    memset(ctr, 0, TWINE_BLOCK_SIZE);
    memcpy(ctr, nonce, nonce_len) /* 8-byte block */;

    while (nbytes > 0) {
        encrypt(ctr, keystream, ctx);

        size_t chunk = (nbytes < TWINE_BLOCK_SIZE) ? nbytes : TWINE_BLOCK_SIZE;
        for (size_t i = 0; i < chunk; i++)
            out[i] = in[i] ^ keystream[i];

        /* 递增计数器 (big-endian, 最后的 ctr_len 字节) */
        for (int i = (int)(TWINE_BLOCK_SIZE - 1); i >= (int)nonce_len; i--) {
            if (++ctr[i] != 0) break;
        }
...
void twine_xts_encrypt(const uint8_t *pt, uint8_t *ct, size_t nbytes,
                 uint64_t data_unit_seq,
                 twine_block_fn encrypt, const void *key1_ctx,
                 twine_block_fn encrypt_tweak, const void *key2_ctx) {
    uint8_t T[16];
...
/* XTS 解密 (同样使用增量 tweak) */
void twine_xts_decrypt(const uint8_t *ct, uint8_t *pt, size_t nbytes,
                 uint64_t data_unit_seq,
                 twine_block_fn encrypt, twine_block_fn decrypt,
